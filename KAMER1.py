    ...
    def TPDL_generate(parametr_proba, parametr_isp, parametr_d, cursor, CM3DATA_OBJID, parametr_press):
        ...
        def graph_tpdl(parametr_press, parametr_isp, vertical_speed_rzg, parametr_proba, vertical_point_uploading):
            ...
            while E != E_now:
                ...
                curve_test_max += 0.000001
                count += 1

                if count > 30000 and abs(E - E_now) > 0.25:
                    curve_test_max += 0.00001

                if count > 60000 and abs(E - E_now) > 0.25:
                    curve_test_max += 0.0001


                if count > 900000 and abs(E - E_now) > 0.25:
                    curve_test_max += 0.001

                if count > 120000 and abs(E - E_now) > 0.25:
                    curve_test_max += 0.01

                if count > 150000 and abs(E - E_now) > 0.25:
                    curve_test_max += 0.1

                if count > 300000 and not E > E_now:
                    curve_test_max += 0.0001
