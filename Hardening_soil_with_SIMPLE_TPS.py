...
def control_isp(parametr_isp, log_dict, parametr_d, parametr_proba, parametr_press, cursor):
    ...
    for st, press, EXAM_NUM, press_name in zip(st_list, press_list, exam_list, press_name):
        ...
        random_start_e1 = randint(3, 9) * -vertical_speed_rzg + 0.00000001
        p_array_1, quantity_point, e1_array, otn_def = HS.graph_tps(st, press, press_name, parametr_d, parametr_isp,
                                                                 vertical_speed_rzg, parametr_proba, random_start_e1)

        list_choise, list_sequance = HS.list_choise1_tps(quantity_point, p_array_1)

        ev_array_1 = HS.create_ev_tps(tps_parametr.get('b'), quantity_point, e1_array)
        if parametr_d.get('not_create_ISP') == 1 or parametr_d.get('write_and_createISP') == 1:
            HS.write_EngGeo_TPS(p_array_1, e1_array, ev_array_1, list_sequance, list_choise, EXAM_NUM, press,
                             quantity_point, CM3DATA_OBJID, cursor)
        if parametr_d.get('not_write_EngGeo') == 1 or parametr_d.get(
                'not_write_EngGeo_not_EngGeo_parametr') == 1 or parametr_d.get('write_and_createISP') == 1:
            HS.ISP_TPS(File_Path, p_array_1, e1_array, ev_array_1, parametr_isp, parametr_proba, press, parametr_d, parametr_press, vertical_speed_rzg, random_start_e1, otn_def, picture_time, EXAM_NUM)
