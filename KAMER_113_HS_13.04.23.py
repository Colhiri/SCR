...
for update_lab in range(1, count_rows_df + 1):
    ...
    if parametr_object.get('diap_activate') == 1:
        ...
        if isp_data.iloc[update_lab][18] != None or isp_data.iloc[update_lab][19] != None:
            parametr_Ro_activate = 1
        ...
        if parametr_Ro_activate == 1 and isp_data.iloc[update_lab][14] == None:
            ...
            if Ro_diap_min == None:
                ...
                if Ro_dispersion == None:
                    ...
                    choise_Ro = randint(((Ro_diap_max - Ro_dispersion / 2) * 100),
                                        ((Ro_diap_max + Ro_dispersion / 2) * 100)) / 100
            ...
            if Ro_diap_max == None:
                ...
                if Ro_dispersion == None:
                    ...
                    choise_Ro = randint(int((Ro_diap_min - Ro_dispersion / 2) * 100),
                                        int((Ro_diap_min + Ro_dispersion / 2) * 100)) / 100
        ...
        if parametr_Ro_activate == 1 and isp_data.iloc[update_lab][14] == None:
            ...
            if Ro_diap_max == None:
                ...
                if Ro_dispersion == None:
                    ...
                    choise_Ro = randint(((Ro_diap_min - Ro_dispersion / 2) * 100),
                                        ((Ro_diap_min + Ro_dispersion / 2) * 100)) / 100
...
def parametr_lab(isp_data):
    ...
    for update_lab in range(1, count_rows_df + 1):
        ...
        Ves = 100.0
        А10 = isp_data.iloc[update_lab][4]
        A5 = isp_data.iloc[update_lab][5]
        A2 = isp_data.iloc[update_lab][6]
        A1 = isp_data.iloc[update_lab][7]
        A05 = isp_data.iloc[update_lab][8]
        A025 = isp_data.iloc[update_lab][9]
        A01 = isp_data.iloc[update_lab][10]
        A005 = isp_data.iloc[update_lab][11]
    ...
    if parametr_write_obj == 0:
        ...
        def search_and_create_skv(cursor, OBJID_OBJECT, parametr_d, parametr_proba, name_table):
            ...
            if OBJID_SKV != None:
                ...
                OBJID_SKV = OBJID()
                cursor.execute(
                    f"INSERT INTO SK (OBJID, ADRESS_HOLE, N_SKV, DIAM, VYRTYPE_OBJID, AWORK_OBJID, NSKV4SORT, DATASOURCE_ID, IS_STAT_SK) VALUES ('{OBJID_SKV}', '{name_table}', '{parametr_proba.get('NO_SKV')}', '0', '4F21D49AC7874F9795EDC4F52E559F55', '{OBJID_OBJECT}', '0000000009', '00000000000000000000000000000000', '0')")
                cursor.commit()
...
#                                      IL, e, Sr])
    ...
    def graph_tps(st, press, press_name, parametr_d, parametr_isp, vert_speed, parametr_proba, random_start_e1):
        ...
        need_tail = randint(0, 1)

        if parametr_proba.get('main_type') == 'incoherent' and need_tail == 1:

            first_point_tail = randint(80, 120) / 1000
            otn_def_incoherent = otn_def.tolist()
            for x in otn_def_incoherent:
                if x > first_point_tail:
                    index = otn_def_incoherent.index(x)
                    break
            definition_tail = randint(1, int((0.15 - first_point_tail) * 100)) / 100
            second_point_tail = first_point_tail + definition_tail
            for x in otn_def_incoherent:
            ...
            random_p = np.asarray([float(randint(0, 100) / 10000) for x in range(len(e1_array))])
            random_p[-1] = 0
            ...
            random_p = np.asarray([float(randint(0, 30) / 10000) for x in range(len(e1_array_tail))])
            random_p[-1] = 0
            ...
            random_p = np.asarray([float(randint(0, 30) / 10000) for x in range(len(e1_array_tail_2))])
            random_p[-1] = 0
    ...
    def create_ev_tps(b, quantity_point, e1_array):
        ...
        if randon_func == 0:
            ...
            for y in e1_array:
                try:
                    ev_array_1.append((a - b * math.exp(-y / c)) ** 2 + d)
                except:
                    ev_array_1.appen
            ev_array_1 = np.asarray(ev_array_1)

            return ev_array_1

        # x=(a - b * math.exp((-y/c))^2 + d

        try:
            if parametr_proba.get('Ip') < 7:
                b += randint(0, 75) / 10
                c = -(b * randint(7, 10) + randint(1, 30))
            ...
            if parametr_proba.get('Ip') >= 7 and parametr_proba.get('Ip') < 17:
                b += randint(0, 50) / 10
                c = -(b * randint(3, 7) + randint(0, 5))
                d = randint(100, 1000)  # сколько прибавить по ev
                random_ev = np.asarray([float(randint(0, 1000) / 100) for x in range(quantity_point)])

            if parametr_proba.get('Ip') >= 17:
                b += randint(0, 20) / 100
                c = -(b * randint(2, 3) + randint(1, 20) / 10)
                d = randint(100, 1000)  # сколько прибавить по ev
                random_ev = np.asarray([float(randint(0, 200) / 100) for x in range(quantity_point)])
        except:
            b = randint(10, 100)
            c = -randint(100, 500)
            d = randint(100, 1000)  # сколько прибавить по ev
            random_ev = np.asarray([float(randint(1500, 3000) / 10) for x in range(quantity_point)])
    ...
    def TPD(parametr_d, parametr_isp, parametr_proba, parametr_press, CM3DATA_OBJID):
        press = parametr_press.get('press')  * 1.95
    ...
    def list_choise_tpd(quantity_point, point_1, point_2, p_array_1):
        # лист с выбором последней точки
    ...
    def TPDL_generate(parametr_proba, parametr_isp, parametr_d, cursor, CM3DATA_OBJID, parametr_press):
        ...
        def graph_tpdl(parametr_press, parametr_isp, vertical_speed_rzg, parametr_proba, vertical_point_uploading):
            ...
            """
            расчет прочностной характеристиики при заданнмо бытовом давлении
            ...
            N = 2 * math.tan(math.pi * parametr_isp.get('F') / 180) * (
                    (((math.tan(math.pi * parametr_isp.get('F') / 180)) ** 2) + 1) ** (1 / 2)) + 2 * (
                        (math.tan(math.pi * parametr_isp.get('F') / 180)) ** 2) + 1
            M = 2 * (N ** (1 / 2)) * parametr_isp.get('C')
            st1 = (parametr_press.get('press') * N + M)


            last_point_2 = st1   # p_array_1[-1] * last_point_koef
            ...
            if parametr_proba.get('main_type') == 'incoherent' and parametr_d.get('need_tail_rzg') == 1:
                ...
                first_point_tail = vertical_point_uploading + randint(dis_point, dis_point + 30) / 1000
                ...
                press_2 = ((last_point_2 * math.atan((e1_array_second[0] / curve) ** 0.5)) / (
                        math.atan((e1_array_second[0] / curve) ** 0.5) - math.atan(
                    (e1_array[index] / curve) ** 0.5))) - (last_point_1 / (((math.atan(
                    (e1_array_second[0] / curve) ** 0.5)) / (math.atan((e1_array_second[-1] / curve) ** 0.5))) - 1))

                c_2 = (last_point_2 / (math.atan(((e1_array[index] / curve)) ** 0.5)) - (
                        press_2 / (math.atan(((e1_array[index] / curve)) ** 0.5))))
        ...
        def list_choise_tpdl(quantity_point, point_1, point_2, p_array_1, points_rzg):
            # лист с выбором последней точки
            list_choise1 = [int(0) for x in range(quantity_point)]  # SELECTED
            list_choise1[point_1] = -1
            list_choise1[point_2] = -1
            ...
            #list_choise1[points_rzg[0]] = -1
            #list_choise1[points_rzg[1]] = -1
            list_choise1[points_rzg[2]] = -1
            list_choise1[points_rzg[3]] = -1
        ...
        def list_choise_NU_tpdl(quantity_point, point_1, point_2, points_rzg):
            # лист по NU
            list_choise_NU_1 = [int(0) for x in range(quantity_point)]  # SELECTED
            list_choise_NU_1[point_1] = -1
            list_choise_NU_1[point_2] = -1

            # list_choise_NU_1[points_rzg[0]] = -1
            # list_choise_NU_1[points_rzg[1]] = -1
            list_choise_NU_1[points_rzg[2]] = -1
            list_choise_NU_1[points_rzg[3]] = -1
            return list_choise_NU_1
        ...
        def write_EngGeo_TPDL(cursor, parametr_proba, parametr_d, parametr_press, p_array_1, e1_array, ev_array_1,
                              list_sequance, list_choise,
        ...
        def summ_dop_line_1(otn_def, p_array_1, e1_array, ev_array_1, parametr_press, quantity_point, points_l,
            ...
            point_1_first_index = line_1_p.tolist().index(nearest(line_1_p, p_array_1[points_l[0]])) + len(p_array_1[:search_point])
            point_2_first_index = line_1_p.tolist().index(nearest(line_1_p, p_array_1[points_l[1]])) + len(p_array_1[:search_point])

            point_1_sec_index = line_2_p.tolist().index(nearest(line_2_p, p_array_1[points_l[0]])) + len(p_array_1[:search_point]) + len(line_1_p)
            point_2_sec_index = line_2_p.tolist().index(nearest(line_2_p, p_array_1[points_l[1]])) + len(p_array_1[:search_point]) + len(line_1_p)
            ...
            points_rzg = [point_1_first_index, point_2_first_index, point_1_sec_index, point_2_sec_index]

            return p_array_1, e1_array, ev_array_1, points_rzg

        p_array_1, e1_array, ev_array_1, points_rzg = summ_dop_line_1(otn_def, p_array_1, e1_array, ev_array_1, parametr_press,
                                                          quantity_point,
        ...
        list_choise1 = list_choise_tpdl(quantity_point, points_l[0], points_l[1], p_array_1, points_rzg)
        list_sequance1 = list_sequance1_tpdl(quantity_point)
        list_choise_NU_1 = list_choise_NU_tpdl(quantity_point, points_l[0], points_l[1], points_rzg)

        if parametr_d.get('not_create_ISP') == 1 or parametr_d.get('write_and_createISP') == 1:
            write_EngGeo_TPDL(cursor, parametr_proba, parametr_d, parametr_press, p_array_1, e1_array, ev_array_1,
                              list_sequance1, list_choise1,
    ...
    def control_isp(parametr_isp, log_dict, parametr_d, parametr_proba, parametr_press, cursor):
        ...
        if parametr_isp.get('SPD') != None:
            moed, p_y, otn_vert_def, NRN, log_dict, press_spd, choise_st = SPD_parametr(parametr_proba, parametr_isp,
                                                                                           log_dict, cursor)
            if parametr_d.get('not_create_ISP') == 1 or parametr_d.get('write_and_createISP') == 1:
