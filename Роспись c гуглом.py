...
def new_dict(lab_in_object, bucs_W, bucs_WL, bucs_WP, rings):
    ...
    for LAB_NO in lab_in_object.keys():
        ...
        if Ip == None:
            ...
            bucs_w0, wet_w0, dry_w0, objid_w0 = NUM, m1, m0, BUCS_OBJID
            bucs_w1, wet_w1, dry_w1, objid_w0 = '', '', '', ''
        else:
            # 1 бюкс
            BUCS_OBJID = random_BUC(bucs_W)
            ...
            bucs_w1, wet_w1, dry_w1, objid_w1 = NUM, m1, m0, BUCS_OBJID
        ...
        if Ip != None:
            ...
            ring_num, soil_weight, ring_objid = NUM, VES1R, RING_OBJID
        else:
            ring_num, soil_weight, ring_objid = '', '', ''
        ...
        if Ip != None:
            ...
            bucs_wl0, wet_wl0, dry_wl0, objid_wl0 = NUM, m1, m0, BUCS_OBJID
            ...
            bucs_wl1, wet_wl1, dry_wl1, objid_wl1 = NUM, m1, m0, BUCS_OBJID

        else:
            bucs_wl0, wet_wl0, dry_wl0, objid_wl0 = '', '', '', ''
            bucs_wl1, wet_wl1, dry_wl1, objid_wl1 = '', '', '', ''
        ...
        if Ip != None:
            ...
            bucs_wp0, wet_wp0, dry_wp0, objid_wp0 = NUM, m1, m0, BUCS_OBJID

        else:
            bucs_wp0, wet_wp0, dry_wp0, objid_wp0 = '', '', '', ''
        ...
        lab_bucs[LAB_NO] = [objid_w0, objid_w1, ring_objid, objid_wl0, objid_wl1, objid_wp0]

    return new_dict, lab_bucs
...
# для записи в ege
def search_ege(cursor, object_name):
    ...
    for skv in skv_in_object:
        ...
        for x in range(len(OBJID_LAB)):
           lab_in_object_ege.setdefault(OBJID_LAB[x][0], OBJID_LAB[x][1])
           lab_OBJID.setdefault(OBJID_LAB[x][1], OBJID_LAB[x][0])

    return lab_in_object_ege, lab_OBJID


def write_W(cursor, new_dict, lab, lab_OBJID, lab_in_object):

    if lab_parametr.get(lab)[5] == None:
        # 1 бюкс
        BUCS_OBJID = lab_bucs.get(lab_OBJID.get(lab))


        m0 = str(new_dict.get(lab_OBJID.get(lab))[4]).replace('.', ',')
        m1 = str(new_dict.get(lab_OBJID.get(lab))[5]).replace('.', ',')
        VES_0 = str((m0*w-100*m1+100*m0)/(lab_in_object.get(lab_OBJID.get(lab)))).replace('.', ',')
        cursor.execute(
    ...
    if lab_parametr.get(lab)[1] != None:
        ...
        VES_0 = str(new_dict.get(lab_OBJID.get(lab))[3]).replace('.', ',')
        m0 = str(new_dict.get(lab_OBJID.get(lab))[4]).replace('.', ',')
        m1 = str(new_dict.get(lab_OBJID.get(lab))[5]).replace('.', ',')
        cursor.execute(
            f"INSERT INTO PROBAGR_W (OBJID, ZAMER_NO, DATA_ISP, BUCS_OBJID, VES0, VES_VLGR, VES_SHGR) VALUES ('{lab}','0', '{directory_time}', '{BUCS_OBJID}', '{VES_0}', '{m1}', '{m0}')")
...
def write_WL(cursor, new_dict, lab, lab_OBJID):
...
def write_WP(cursor, new_dict, lab, lab_OBJID):
...
def write_RING(cursor, new_dict, lab, lab_OBJID):
...
def write_GRANS(cursor, new_dict, lab, lab_OBJID):
...
try:
    ...
    name_base = 'D:/Резервное копирование/HETA.mdb' # my_file.read()
    # my_file.close()
    # print(name_base)
    ...
    wbxl = xw.Book('D:/Резервное копирование/Object.xlsx')
    ...
    new_dict, lab_bucs = new_dict(lab_in_object, bucs_W, bucs_WL, bucs_WP, rings)
    ...
    sorted_df.to_csv(f"{object}_.log", sep='\t', index=False, header=False)
